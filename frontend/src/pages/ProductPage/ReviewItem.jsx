...
export const ReviewItem = ({ review, isOwner }) => {
  ...
  const displayName =
    review.transactionId?.orderId?.shippingAddress?.name ||
  ...
  return (
    <div className="py-6 mb-3 bg-gray-100 rounded-lg px-4">
      <div className="flex items-start mb-3">
        ...
        <div className="text-right flex-shrink-0 ml-auto">
          ...
          {isOwner && (
            <div className="mt-2">
              {" "}
              {/* Margin-top to place it 'under' */}
              <ReviewReport reviewId={review._id} />
            </div>
          )}
        </div>
      </div>
      ...
      {/* Review Comment */}
      <p className="text-gray-600 leading-relaxed pl-13">{reviewText}</p>
      {isLongReview && (
        <button
