...
const useOrderData = (page) => {
  ...
  const filteredOrders = useMemo(() => {
    ...
    switch (page) {
      ...
      case 5:
        return orders.filter((item) =>
          getLatestStatusKey(item).includes("CANCELLED")
        );
      ...
      case 7:
        return orders.filter((item) =>
          getLatestStatusKey(item).includes("PICKED_UP")
        );
  ...
  const submitReturnRequest = async (orderId, products, reason) => {
    try {
      ...
      alert(
        err.message ||
          "Failed to submit return request. Check console for details."
      );
      return null;
    }
  };
  ...
  const submitTrackingId = async (orderId, trackingId) => {
    try {
      ...
      alert(
        `Tracking ID ${trackingId} for Order ${orderId} submitted successfully!`
      );
      return result;
    } catch (err) {
